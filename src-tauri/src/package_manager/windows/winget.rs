...
impl PackageManager for Winget {
    ...
    async fn search(&self, query: &str) -> Result<Vec<AppModel>, String> {
        ...
        for line in stdout.lines() {
            ...
            if !parsing_data || line.trim().is_empty() {
                continue;
            }
    ...
    async fn list_upgradable(&self) -> Result<Vec<AppModel>, String> {
        ...
        for line in stdout.lines() {
            ...
            if !parsing_data || line.trim().is_empty() || line.contains("upgrades available") {
                continue;
            }
            ...
            if parts.len() >= 4 {
                let len = parts.len();
                let _source = parts[len - 1];
                let new_version = parts[len - 2];
                let _old_version = parts[len - 3];
                let id = parts[len - 4];
                ...
                results.push(AppModel {
                    id: id.to_string(),
                    name,
                    description: format!("Update available to {}", new_version),
                    publisher: "Unknown".to_string(),
